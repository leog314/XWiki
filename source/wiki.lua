...
function HomeScreen:paint(gc)
    ...
    local img
    if white_mode then
        img = image.new(images["settings-icon-white_mode"])
    else
        img = image.new(images
            ["settings-icon-dark_mode"])
    end
    ...
    gc:drawRect(self.shiftx + self.search_bar.x0, self.shifty + self.search_bar.y0, self.search_bar.x1 -
        self.search_bar.x0, self.search_bar.y1 - self.search_bar.y0)

    if self.search_bar.text == self.placeholder then -- editor content
        if white_mode then
            gc:setColorRGB(uCol(colors["placeholder"]))
        else
            gc:setColorRGB(uInvertCol(colors
                ["placeholder"]))
        end
    else
    ...
    for i = 1, #self.articles do
        ...
        if keyword ~= nil then
            ...
            gc:drawRect(self.shiftx + self.search_bar.x0, self.shifty + y, self.search_bar.x1 - self.search_bar.x0,
                math.round(self.article_box_height) - 1)
...
function HomeScreen:updateSearch()
    if self.search_bar.text ~= self.placeholder then
        ...
        self.pointer = math.min(self.pointer, #self.articles)
    else
        self.articles = {}
        self.pointer = 0
    end
...
function HomeScreen:paste()
    if self.search_bar.text ~= self.placeholder then
        self.search_bar.text = self.search_bar.text .. clipboard.getText()
    else
